...
vec3 colorFunc(float n) {
    // -- COLOR FUNC
    float r = sin((mod(n, 0.25) * 4.0 + u_time * 0.5) * 2.0 * PI) +
              sin(mod(u_time / 9.0, 1.0) * 2.0 * PI);
    float g = sin((mod(n, 0.3333) * 3.0 + u_time * 0.3) * 2.0 * PI) +
              sin(mod(u_time / 13.0, 1.0) * 2.0 * PI);
    float b = sin((mod(n, 0.5) * 2.0 + u_time * 0.5) * 2.0 * PI) +
              sin(mod(u_time / 19.0, 1.0) * 2.0 * PI);
...
void main() {
    ...
    float scroll_speed = SCROLL_SPEED_Z + sin(u_time * 0.2) * 0.0005;
